...
$pdf->Cell(74, 5, strtoupper($row['fullname']), 'B', 0);
$pdf->SetFont('Arial', '', 10);
...
$pdf->SetXY(10, 143.5);
$l = mysqli_query($conn, "SELECT * FROM tbl_students WHERE stud_id = '$stud_id'");
while ($rows = mysqli_fetch_array($l)) {

    $sum = mysqli_query($conn, "SELECT SUM(unit_total) as UN FROM tbl_enrolled_subjects LEFT JOIN tbl_subjects_new ON tbl_subjects_new.subj_id = tbl_enrolled_subjects.subj_id where tbl_enrolled_subjects.stud_id = '$stud_id' AND tbl_enrolled_subjects.acad_year = '$acadyear' AND tbl_enrolled_subjects.semester = '$semester'");
    ...
    $sqls = mysqli_query($conn, "SELECT *,tbl_subjects_new.subj_code,tbl_subjects_new.subj_desc FROM tbl_enrolled_subjects LEFT JOIN tbl_subjects_new ON tbl_subjects_new.subj_id = tbl_enrolled_subjects.subj_id where tbl_enrolled_subjects.acad_year = '$acadyear' AND tbl_enrolled_subjects.semester = '$semester' AND stud_id = '$stud_id'") or die($conn);
    $y = $pdf->Gety();
    $xy = 6.5;
    $i = 1;
...
$pdf->SetFont('Arial', 'B', 9);
$pdf->Cell(79, 5, 'Sheryl D. Sajo', 0, 0, 'C');
$pdf->SetFont('Arial', '', 9);
